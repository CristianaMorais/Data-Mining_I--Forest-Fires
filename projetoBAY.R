...
aux.bay <- aux.bay %>% select(-c("id","parish","total_area", "village_area", "village_veget_area","alert_hour","municipality"))
...
aux.bay$lat <- factor(aux.bay$lat ,ordered=TRUE)
aux.bay$lon <- factor(aux.bay$lon ,ordered=TRUE)
...
aux2.bay <- aux2.bay %>% select(-c("id","parish","total_area","village_veget_area","village_area","alert_hour","municipality"))
